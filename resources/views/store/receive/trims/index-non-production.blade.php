...
@section('title')
    Non Production Trims Receive Transaction
@endsection
@section('content')
    <style type="text/css">
        /* th{
        ...
        <div class="pageheader">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="{{route('store.receive.trims.finished-non-production')}}"> Non Production Trims Item Receive</a>
                    </li>
                    <li>
                        <a href="#"> Non Production Trims Item Receive Transactions</a>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">                <!-- tile -->
                <section class="tile">
                    ...
                    <div class="tile-header dvd dvd-btm">
                        <h1 class="custom-font"><strong>Non Production Trims Item</strong> Receive Transaction</h1>
                        <ul class="controls">
                            <li class="dropdown">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover table-bordered table-condensed" id="advanced-usage">
                                            ...
                                            @foreach($activeProductionPlans as $item)
                                                @if($item->non_production_replace_quantity > 0)
                                                    <tr>
                                                        ...
                                                        <td class="text-center">{!! (\App\Helpers\Helper::IDwiseData('units', 'id', (\App\Helpers\Helper::TwoIDwiseData('delivery_details', 'delivery_master_id', $item->delivery_master_id, 'item_count', $item->delivery_detail_id))->gross_unit))->short_unit !!}</td>
                                                        <td class="text-center">{!! $item->non_production_replace_quantity !!}</td>
                                                        <td class="text-center">
                                                           {{-- @if(Auth::user()->hasTaskPermission('rejectfinishedtrims', Auth::user()->id))
                                                                @if( (\App\Helpers\Helper::GetCurrentLeftOverTrimsStock(\App\Helpers\Helper::StockIDBasedOnPO($item->purchase_order_master_id,  $item->purchase_order_detail_id))) >= $item->left_over_production)
--}}{{--                                                                <a title="Reject This Left Trims Item" class="DeActivateBuyer btn btn-danger btn-xs" data-id = "{{ $item->id }}"><i class="fa fa-arrow-circle-down"></i></a>--}}{{--
                                                                @endif
                                                            @endif--}}
