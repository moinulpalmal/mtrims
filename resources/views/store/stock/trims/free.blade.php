        ...
        <div class="pageheader">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="{{route('store.stock.free-trims.current')}}"> Free Current Stock</a>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">                <!-- tile -->
                <section class="tile">
                    ...
                    <div class="tile-header dvd dvd-btm">
                        <h1 class="custom-font"><strong>Trims Item</strong> Free Stock List</h1>
                        <ul class="controls">
                            <li class="dropdown">
...
{{--                                                                <button title="Update This Plan" class="btn btn-warning btn-xs" data-toggle="modal" data-target="#plan{{$item->id}}" data-options="splash-2 splash-ef-12"><i class="fa fa-edit"></i></button>--}}
                                                                <button title="Generate Left Over Request" class="btn btn-info btn-xs" data-toggle="modal" data-target="#left{{$item->id}}" data-options="splash-2 splash-ef-12"><i class="fa fa-eye"></i></button>
                                                            </td>
                                                        </tr>
                                                @endforeach
                                                </tbody>
                            ...
                            <div class="row" style="padding: 0px 15px;">
                                ...
                                <div class="col-md-6 no-padding">
                                    <div class="form-group">
                                        <label for="DRPQty{{$item->id}}" class="control-label">Current Free Qty</label>
                                        <input type="number" step="any" class="form-control" name="current_free_quantity" id="DRPQty{{$item->id}}" required value="{{ old('current_free_quantity', $item->stock_quantity)}}" readonly>
                                    </div>
                                </div>
                                <div class="col-md-6 no-padding">
                                    <div class="form-group">
                                        <label for="DRQty{{$item->id}}" class="control-label">Left Over Request Qty</label>
                                        <input type="number" step="any" class="form-control" name="item_request_qty" id="DRQty{{$item->id}}" max="{{$item->stock_quantity}}" required value="{{ old('item_request_qty')}}">
                                    </div>
                                </div>
                            ...
                            <div class="row" style="padding: 0px 15px;">
                                <div class="col-md-12 no-padding">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="replacement_reason" id="ReplacementReason{{$item->id}}" required value="{{ old('replacement_reason') }}">
                                    </div>
                                </div>
                            </div>
                            <div class="row" style="padding: 0px 15px;">
                                <div class="col-md-12 no-padding">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="remarks" id="Remarks{{$item->id}}" value="{{ old('remarks') }}">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(function(){
            ...
            $('#LeftRequest{{$item->id}}').submit(function(e){
                ...
                var url = '{{ route('store.stock.free-trims.request-left-over') }}';
                //console.log(data);
                //return;
                swal({
                    title: 'Are you sure?',
                    text: 'You want to generate request for transfer to left over stock!',
                    icon: 'warning',
                    buttons: ["Cancel", "Yes!"],
                }).then(function(value) {
                    if (value) {
