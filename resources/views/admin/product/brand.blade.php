...
@section('content')
    ...
    <div class="page page-dashboard">
        <div class="pageheader ">
            <h2>Brand Name <span>Brand Name List</span></h2>
            <div class="page-bar">
        ...
        <div class="row">
                ...
                <form method="post" id="FactoryAdd" enctype="multipart/form-data">
                    ...
                    <section class="tile">
                        ...
                        <div class="tile-header dvd dvd-btm">
                            <h1 class="custom-font"><strong>Product Brand</strong> Insert/Update Form</h1>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <section class="tile">
                    <!-- tile header -->
                    <div class="tile-header dvd dvd-btm">
                        <h1 class="custom-font"><strong>Product Brand</strong> List</h1>
                        <ul class="controls">
                            <li class="dropdown">
    ...
    <script>
        ...
        function loadDataTable() {
           ...
           // $('tbody').html(free_table);
            table = $("#advanced-usage").DataTable({
                ajax: {
                    url: "/mtrims/public/api/admin/product-brand/not-deleted",
                    dataSrc: ""
                },
                columns: [
                    {
        ...
        $(function(){
            ...
            $('#FactoryAdd').submit(function(e){
                ...
                // console.log(data);
                var url = '{{ route('admin.product.brand.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        $('#advanced-usage').on('click',".EditFactory", function(){
            ...
            var url = '{{ route('admin.product.brand.edit') }}';
            $.ajax({
                url: url,
                method:'POST',
                data:{id: FactoryID},
        ...
        $('#advanced-usage').on('click',".ActivateBuyer", function(){
            ...
            var url = '{{ route('admin.product.brand.activate') }}';
            swal({
                title: 'Are you sure?',
                text: 'This product brand will be a active one!',
                icon: 'warning',
                buttons: ["Cancel", "Yes!"],
            }).then(function(value) {
                if (value) {
        ...
        $('#advanced-usage').on('click',".DeActivateBuyer", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('admin.product.brand.de-activate') }}';
            swal({
                title: 'Are you sure?',
                text: 'This product brand will be in-active!',
                icon: 'warning',
                buttons: ["Cancel", "Yes!"],
            }).then(function(value) {
                if (value) {
        ...
        $('#advanced-usage').on('click',".DeleteBuyer", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('admin.product.brand.delete') }}';
            swal({
                title: 'Are you sure?',
                text: 'This product brand will be removed permanently!',
                icon: 'warning',
                buttons: ["Cancel", "Yes!"],
            }).then(function(value) {
                if (value) {
