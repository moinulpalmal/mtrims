...
@section('title')Product Setup @endsection
@section('content')
    <style type="text/css">
        th {
    ...
    <div class="page page-dashboard">
        <div class="pageheader ">
            <h2>Product Setup <span>Product Setup List</span></h2>
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="{{route('admin.product.setup')}}"> Product Setup</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <form method="post" id="FactoryAdd" enctype="multipart/form-data">
                    ...
                    <section class="tile">
                        ...
                        <div class="tile-header dvd dvd-btm">
                            <h1 class="custom-font"><strong>Product Setup</strong> Insert/Update Form</h1>
                            ...
                            <div class="row" style="padding: 0px 15px;">
                                ...
                                <div class="col-md-2 no-padding">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" name="category_name" id="CategoryName" style="width: 100% !important; height: 100% !important;" required>
                                ...
                                <div class="col-md-2 no-padding">
                                    <div class="form-group">
                                        <label for="UnitName" class="control-label">Select Unit</label>
                                        <select class="form-control select2" name="unit_name" id="UnitName" style="width: 100% !important; height: 100% !important;" required>
                                            <option value="" selected="selected">- - - Select Unit - - -</option>
                                                @if(!empty($units))
                                                    @foreach($units as $unit)
                                                        <option value="{{ $unit->id }}">{{ $unit->full_unit }}</option>
                                                    @endforeach
                                                @endif
                                        </select>
                                    </div>
    ...
    <script>
        ...
        function loadDataTable() {
           ...
           // $('tbody').html(free_table);
            table = $("#advanced-usage").DataTable({
                ajax: {
                    url: "/mtrims/public/api/admin/product-setup/not-deleted",
                    dataSrc: ""
                },
                columns: [
                    {
        ...
        $(function(){
            ...
            $('#FactoryAdd').submit(function(e){
                ...
                // console.log(data);
                var url = '{{ route('admin.product.setup.save') }}';
                //console.log(data);
                $.ajax({
                    url: url,
                    method:'POST',
        ...
        $('#advanced-usage').on('click',".EditFactory", function(){
            ...
            var url = '{{ route('admin.product.setup.edit') }}';
            $.ajax({
                url: url,
                method:'POST',
                data:{id: FactoryID},
                success:function(data){
                    $('select[name=brand_name]').val(data.brand_name).change();
                    $('select[name=category_name]').val(data.category_name).change();
                    $('select[name=unit_name]').val(data.unit_name).change();
        ...
        $('#advanced-usage').on('click',".ActivateBuyer", function(){
            ...
            var url = '{{ route('admin.product.setup.activate') }}';
            swal({
                title: 'Are you sure?',
                text: 'This product will be a active one!',
                icon: 'warning',
                buttons: ["Cancel", "Yes!"],
            }).then(function(value) {
                if (value) {
        ...
        $('#advanced-usage').on('click',".DeActivateBuyer", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('admin.product.setup.de-activate') }}';
            swal({
                title: 'Are you sure?',
                text: 'This product will be in-active!',
                icon: 'warning',
                buttons: ["Cancel", "Yes!"],
            }).then(function(value) {
                if (value) {
        ...
        $('#advanced-usage').on('click',".DeleteBuyer", function(){
            var button = $(this);
            var id = button.attr("data-id");
            var url = '{{ route('admin.product.setup.delete') }}';
            swal({
                title: 'Are you sure?',
