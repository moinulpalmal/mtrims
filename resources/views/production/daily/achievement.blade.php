        ...
        <div class="pageheader ">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#"> {{\Carbon\Carbon::parse($date)->format('d/m/Y')}}</a>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">                <!-- tile -->
                <section class="tile">
                    ...
                    <div class="tile-header dvd dvd-btm">
                        <h1 class="custom-font"><strong>Production Plan Achievement</strong> List</h1>
                        <ul class="controls">
                            <li class="dropdown">
...
{{--                                                    <th class="text-center">Batch No</th>--}}
                                                    ...
                                                    <th class="text-center">Achievement Qty</th>
                                                    <th class="text-center">Left-Over Qty</th>
                                                    <th class="text-center">Variation Qty</th>
                                                ...
                                                @foreach($activeProductionPlans as $item)
                                                    @if($itemTrims->id == (\App\Helpers\Helper::IDwiseData('trims_types', 'id', (\App\Helpers\Helper::TwoIDwiseData('purchase_order_details', 'purchase_order_master_id', $item->purchase_order_master_id, 'item_count', $item->purchase_order_detail_id))->trims_type_id))->section_setup_id)
                                                        @if($item->status != 'SI')
                                                            <tr>
...
{{--                                                                <td class="text-center">{{$item->purchase_order_master_id}}{{$item->purchase_order_detail_id}}{{$item->id}}</td>--}}
                                                                ...
                                                                <td class="text-center">{!! $item->achievement_production !!}</td>
                                                                <td class="text-center">{!! $item->left_over_production !!}</td>
                                                                <td class="text-center">{!! $item->target_production-$item->achievement_production !!}</td>
                                                                <td class="text-center">
                                                                    @if(Auth::user()->hasTaskPermission('ppachievement', Auth::user()->id))
                                                                        @if($item->is_stored == false)
...
{{--                                                    <td class="text-center">Batch No</td>--}}
                                                    ...
                                                    <td class="text-center">Target Qty</td>
                                                    <td class="text-center">Achievement Qty</td>
                                                    <td class="text-center">Left Over Qty</td>
                                                    <td class="text-center">Variation Qty</td>
                ...
                <form method="post" name="PPGenerateForm{{$item->id}}" id="PPGenerate{{$item->id}}" enctype="multipart/form-data">
                    ...
                    <div class="modal-content">
                        <div class="modal-header bg-greensea">
                            <h3 class="modal-title custom-font text-white">Production Plan Achievement Form</h3>
                        </div>
                            ...
                            <div class="row" style="padding: 0px 15px;">
                                ...
                                <div class="col-md-3 no-padding">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="any" class="form-control" name="target_production" id="Target{{$item->id}}" readonly required value="{{old('target_production', $item->target_production)}}">
                                    </div>
                                </div>
                                <div class="col-md-3 no-padding">
                                    <div class="form-group">
                                        <label for="Achievement{{$item->id}}" class="control-label">Achievement Production</label>
                                        <input type="number" step="any" class="form-control" name="achievement_production" id="Achievement{{$item->id}}" required max="$item->target_production" value="{{old('achievement_production', $item->achievement_production)}}">
                                    </div>
                                </div>
                            </div>
                            <div class="row" style="padding: 0px 15px">
                                <div class="col-md-3 no-padding">
                                    <div class="form-group">
                                        <label for="LeftOver{{$item->id}}" class="control-label">Left Over Production</label>
                                        <input type="number" step="any" class="form-control" name="left_over_production" id="LeftOver{{$item->id}}" value="{{old('left_over_production', $item->left_over_production)}}">
                                    </div>
                                </div>
                        ...
                        <div class="modal-footer">
                            <a><button class="btn btn-success btn-ef btn-ef-3 btn-ef-3c" type="submit"><i class="fa fa-arrow-right"></i> Submit Achievement</button></a>
    ...
    <script>
        ...
        $(function(){
            ...
            $('#PPGenerate{{$item->id}}').submit(function(e){
                ...
                var achievement_production = document.forms["PPGenerateForm{{$item->id}}"]["achievement_production"].value;
                var variation_production =  parseFloat(target_production).toFixed(5) - parseFloat( achievement_production).toFixed(5);
                ...
                if(delivery_location_id === ""){
                ...
                else if(achievement_production <= 0){
                    swal({
                        title: "Invalid Achievement Production!",
                        icon: "warning",
                        button: "Ok!",
                    });
                    return false;
                    ...
                    var url = '{{ route('production.plan.daily.achievement.save') }}';
                    ...
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            if(parseInt(data) === 1){
                                ...
                                swal({
                                    title: "Invalid production entry!",
                                    icon: "warning",
                                    button: "Ok!",
                                });
                                return false;
